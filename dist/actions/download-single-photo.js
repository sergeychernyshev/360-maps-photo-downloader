...
async function downloadSinglePhoto(req, photo) {
    const progressCallback = (progress) => {
        (0, download_state_1.updateState)({ ...progress, photoId: photo.photoId.id });
    };
    try {
        ...
        const result = await (0, photo_processor_1.processPhoto)(drive, oAuth2Client, photo, folderId, progressCallback);
        if (result) {
            ...
            if (req.session.missingPhotos &&
                ...
                const missingIndex = req.session.missingPhotos.findIndex((p) => p.photoId.id === photo.photoId.id);
                if (missingIndex > -1) {
                    req.session.missingPhotos.splice(missingIndex, 1);
                }
                // Remove from downloadedPhotos if it exists (for re-downloads)
                const downloadedIndex = req.session.downloadedPhotos.findIndex((p) => p.photoId.id === photo.photoId.id);
            ...
            (0, download_state_1.updateState)({
                ...
                totalPhotosCount: req.session.downloadedPhotos.length +
