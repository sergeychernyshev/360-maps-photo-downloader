...
async function downloadAllPhotos(req, photos, downloadedPhotosCount, missingPhotosCount) {
    ...
    try {
        ...
        const initialProgress = totalPhotoCount > 0
        ...
        for (let i = 0; i < photos.length; i++) {
            ...
            if (existingFileNames.has(fileName)) {
                ...
                downloadedPhoto = await (0, photo_processor_1.processPhoto)(drive, oAuth2Client, photo, folderId, progressCallback);
            }
            if (downloadedPhoto) {
                if (req.session.missingPhotos &&
                    req.session.downloadedPhotos) {
                    const downloadedPhotoIndex = req.session.missingPhotos.findIndex((p) => p.photoId.id === photo.photoId.id);
                    if (downloadedPhotoIndex > -1) {
                        const [splicedPhoto] = req.session.missingPhotos.splice(downloadedPhotoIndex, 1);
                ...
                (0, download_state_1.updateState)({
                    ...
                    totalProgress: Math.round(((downloadedPhotosCount + i + 1) /
                ...
                skippedCount++;
                if (req.session.missingPhotos) {
                    const skippedPhotoIndex = req.session.missingPhotos.findIndex((p) => p.photoId.id === photo.photoId.id);
