...
async function handleMessage(req, ws, message) {
    ...
    switch (type) {
        case "get-state":
            ws.send(JSON.stringify({ type: "progress", payload: (0, download_state_1.getState)() }));
            break;
        case "download":
            ...
            await (0, download_all_photos_1.downloadAllPhotos)(req, missingPhotos, downloadedPhotos.length, missingPhotos.length);
            break;
        ...
        case "download-photo":
            ...
            if (photo) {
                ...
                (0, download_state_1.updateState)({ error: `Photo with ID ${payload.photoId} not found.` });
            }
            break;
