...
async function initialize() {
        ...
        if (config.save_token) {
            ...
            res.on("finish", async () => {
                if (req.session &&
                    ...
                    await fs_1.promises.writeFile(TOKEN_PATH, JSON.stringify(req.session.tokens, null, 2));
    ...
    app.use("/modules/leaflet", express_1.default.static(path_1.default.join(process.cwd(), "node_modules/leaflet/dist")));
    app.use("/modules/leaflet.markercluster", express_1.default.static(path_1.default.join(process.cwd(), "node_modules/leaflet.markercluster/dist")));
    ...
    async function startServer() {
        try {
            ...
            console.error("   Please ensure the credentials file from Google Cloud is in the project directory.");
            process.exit(1);
        }
    }
    startServer();
