...
function updateMap(photos) {
    ...
    photos.forEach((photo) => {
        if (photo.pose && photo.pose.latLngPair) {
            ...
            marker.bindPopup(`<a href="${photo.shareLink}" target="_blank">${photo.places?.[0]?.name || photo.photoId.id}</a>`);
...
function confirmDownload() {
    ...
    document.getElementById("download-fieldset").style.display =
        "block";
    connectWebSocket();
    if (ws && ws.readyState === WebSocket.OPEN) {
...
function updatePhotoList() {
    ...
    document
        .querySelectorAll('#search-input, #reset-filters-btn, .status-filter a, #more-filters-btn, .pose-filter-group, #download-all-btn, form button[type="submit"]')
...
function connectWebSocket() {
    ...
    ws.onmessage = (event) => {
        ...
        if (data.type === "filter-results") {
            const { photoListHtml, paginationHtmlTop, paginationHtmlBottom, poseCounts, downloadedCount, notDownloadedCount, totalPhotosCount, startIndex, endIndex, filteredTotal, currentPage, totalPages, requestPayload, } = data.payload;
            // 1. Update DOM
            const updateBtn = document.getElementById("update-btn");
            ...
            document.getElementById("download-all-btn").disabled = notDownloadedCount === 0;
            if (filteredTotal > 0) {
            ...
            if (!requestPayload.isPopState) {
                ...
                if (requestPayload.search)
                    params.set("search", requestPayload.search);
        ...
        if (data.type === "progress") {
            ...
            if (global) {
                const { message, totalProgress, downloadProgress, fileComplete, downloadedCount, notDownloadedCount, complete, error, status, inProgress, } = global;
                if (inProgress) {
                ...
                if (fileComplete) {
                    const { downloadedCount, notDownloadedCount, totalPhotosCount } = global;
                    document.getElementById("downloaded-count").textContent =
                        downloadedCount;
                    document.getElementById("not-downloaded-count").textContent =
                        notDownloadedCount;
                    ...
                    if (totalPhotosCount !== undefined) {
                        ...
                        document.getElementById("all-count").textContent = (downloadedCount + notDownloadedCount).toString();
                    }
                }
                if (complete || error) {
            ...
            if (individual) {
                for (const photoId in individual) {
                    ...
                    const { downloadProgress, uploadProgress, uploadStarted, complete, error, driveLink, } = photoState;
                    const row = document.querySelector(`tr[data-photo-id="${photoId}"]`);
                    if (row) {
                        const statusCell = row.querySelector(".status-cell");
                        const actionsCell = row.querySelector(".actions-cell");
                        const progressCell = row.querySelector(".progress-cell");
                        const progressBarContainer = progressCell.querySelector(".progress-bar-container");
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.body.addEventListener("click", (event) => {
        ...
        if (pageBtn) {
            ...
            const location = pageBtn.closest(".pagination").dataset
                .location;
