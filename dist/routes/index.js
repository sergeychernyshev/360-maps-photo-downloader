  ...
  try {
    ...
    if (loggedIn) {
      ...
      if (req.session.allPhotos) {
        ...
        let photoListFile = await (0, drive_manager_1.getPhotoListFile)(
          drive,
          folderId,
        );
        if (photoListFile) {
          photos = await (0, drive_manager_1.readFileContent)(
            drive,
            photoListFile.id,
          );
        } else {
    ...
    const driveFiles =
      loggedIn && drive && folderId
        ? await (0, drive_manager_1.listFiles)(drive, folderId)
        : [];
    const drivePhotoCount = driveFiles.filter(
      (f) => f.name !== drive_manager_1.PHOTO_LIST_FILE_NAME,
    ).length;
    ...
    const driveOnlyFiles = driveFiles.filter(
      (f) =>
        f.name !== drive_manager_1.PHOTO_LIST_FILE_NAME &&
        !photoIdsFromStreetView.has(f.name),
    );
    ...
    const paginationHtmlTop = (0, photo_utils_1.buildPaginationHtml)(
      totalPages,
      page,
      "changePage",
      "top",
    );
    const paginationHtmlBottom = (0, photo_utils_1.buildPaginationHtml)(
      totalPages,
      page,
      "changePage",
      "bottom",
    ...
    res.render("index", {
      ...
      photoListHtml: (0, photo_utils_1.buildPhotoListHtml)(
        paginatedPhotos,
        downloadedFiles,
        driveFileLinks,
      ),
