...
exports.degToDmsRational = degToDmsRational;
exports.calculatePoseCounts = calculatePoseCounts;
exports.buildPhotoListHtml = buildPhotoListHtml;
exports.buildPaginationHtml = buildPaginationHtml;
/**
...
function buildPhotoListHtml(photos, downloadedFiles, driveFileLinks) {
    return photos
        ...
        if (photo.pose) {
            if (typeof photo.pose.heading === "number")
                poseParts.push(`<span style="white-space: nowrap;" title="Heading: ${photo.pose.heading.toFixed(2)}°"><strong>H</strong> ${photo.pose.heading.toFixed(2)}</span>`);
            if (typeof photo.pose.pitch === "number")
                poseParts.push(`<span style="white-space: nowrap;" title="Pitch: ${photo.pose.pitch.toFixed(2)}°"><strong>P</strong> ${photo.pose.pitch.toFixed(2)}</span>`);
            if (typeof photo.pose.roll === "number")
                poseParts.push(`<span style="white-space: nowrap;" title="Roll: ${photo.pose.roll.toFixed(2)}°"><strong>R</strong> ${photo.pose.roll.toFixed(2)}</span>`);
            if (typeof photo.pose.altitude === "number")
                poseParts.push(`<span style="white-space: nowrap;" title="Altitude: ${photo.pose.altitude.toFixed(2)}m"><strong>A</strong> ${photo.pose.altitude.toFixed(2)}</span>`);
        }
        ...
        const placeId = photo.places && photo.places.length > 0 && photo.places[0].placeId;
        ...
        const photoIdHtml = locationName
            ? `<a href="https://www.google.com/maps/place/?q=place_id:${placeId}" target="_blank">${locationName}</a><br><small><a href="${photo.shareLink}" target="_blank">${photo.photoId.id}</a></small>`
    ...
    <tr data-photo-id="${photo.photoId.id}">
      ...
      <td class="actions-cell">
        <button data-photo-id="${photo.photoId.id}" class="button download-single-btn ${isDownloaded ? "redownload-btn" : "download-btn"}" style="font-size: 12px; padding: 5px 10px;" title="${isDownloaded ? "Re-download" : "Download"}">
...
//# sourceMappingURL=photo-utils.js.map