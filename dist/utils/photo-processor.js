 ...
 */
async function processPhoto(drive, oAuth2Client, photo, folderId, progressCallback) {
    let attempts = 0;
    const maxAttempts = 3;
    let jpegData;
    let newJpeg;
    while (attempts < maxAttempts) {
        try {
            ...
            const { data } = await (0, photo_manager_1.downloadPhoto)(photo.downloadUrl, oAuth2Client, (percentage) => {
                progressCallback({
            ...
            if (photo.pose) {
                ...
                const gpsData = {
                    ...
                    [piexifjs_1.default.GPSIFD.GPSLatitude]: (0, photo_utils_1.degToDmsRational)(Math.abs(lat)),
                    [piexifjs_1.default.GPSIFD.GPSLongitudeRef]: lng < 0 ? "W" : "E",
                    [piexifjs_1.default.GPSIFD.GPSLongitude]: (0, photo_utils_1.degToDmsRational)(Math.abs(lng)),
                };
    ...
    const existingFile = await (0, drive_manager_1.findFile)(drive, fileName, folderId);
    let file;
    progressCallback({ uploadStarted: true, photoId: photo.photoId.id });
    (0, download_state_1.updateState)({ status: "uploading" });
    if (existingFile) {
        file = await (0, drive_manager_1.updateFile)(drive, existingFile.id, "image/jpeg", stream, newJpeg.length, (percentage) => {
            progressCallback({
        ...
        file = await (0, drive_manager_1.createFile)(drive, fileName, "image/jpeg", stream, folderId, newJpeg.length, (percentage) => {
            progressCallback({
                uploadProgress: percentage,
                photoId: photo.photoId.id,
            });
