...
- When the user says 'new feature' followed by a name, I will create a new Git branch with ticket number (if known), as a prefix, then '/', than feature name, from the 'main' branch. I will use hyphens to separate words in the branch name.
- When I start and current project branch is not 'main', read the branch name and extract ticket number to determine which ticket we are currently working on.
